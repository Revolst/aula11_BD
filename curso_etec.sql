...
/*5 A - JOIN - LEFT JOIN - RIGHT JOIN*/

select a.id_are 'Id Área', a.nm_are 'Nome da Área' ,c.id_cur 'Id Curso' , c.nm_cur 'Curso'
from area a join curso c on a.id_are = c.id_cur;

select a.id_are 'Id Área', a.nm_are 'Nome da Área' , c.nm_cur 'Curso'
from area a left join curso c on a.id_are = c.id_cur;

select a.id_are 'Id Área', a.nm_are 'Nome da Área' , c.nm_cur 'Curso'
from area a right join curso c on a.id_are = c.id_cur;

/* select c.id_cur 'ID Curso', c.nm_cur 'Nome do Curso', a.id_are 'Id Área', a.nm_are 'Nome da Área'
from curso c left join area a on c.id_cur = a.id_are;*/


/* 5 B JOIN - LEFT JOIN - RIGHT JOIN */

select a.id_are 'Id Área', a.nm_are 'Nome da Área' , c.nm_cur 'Curso', p.nm_prof 'Professor'
...
/*5 C JOIN - LEFT JOIN - RIGHT JOIN */

select a.id_are 'Id Area', a.nm_are 'Nome da Area' , c.nm_cur 'Curso', p.nm_prof 'Professor'
from area a join curso c on a.id_are = c.id_are
			join professor p on c.id_cur = p.id_cur
order by a.id_are;
...
/* 5 D JOIN - LEFT JOIN - RIGHT JOIN */

select a.id_are 'Id Area', a.nm_are 'Nome da Area' , c.nm_cur 'Curso', p.nm_prof 'Professor', m.nm_mat 'Nome da Matéria', m.sg_mat 'Sigla da Materia' 
from area a join curso c on a.id_are = c.id_are
			join professor p on c.id_cur = p.id_cur
...
select a.id_are 'Id Area', a.nm_are 'Nome da Area' , c.nm_cur 'Curso', p.nm_prof 'Professor', m.nm_mat 'Nome da Materia', m.sg_mat 'Sigla da Materia' 
from area a right join curso c on a.id_are = c.id_are
			right join professor p on c.id_cur = p.id_cur
			right join materia m on m.id_prof = p.id_prof 
			where a.id_are = 1
